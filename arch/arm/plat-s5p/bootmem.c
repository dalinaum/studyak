...
dma_addr_t s3c_get_media_memory_bank(int dev_id, int bank)
{
	...
	if (!mdev) {
		printk(KERN_ERR "%s : invalid media device dev_id[%d], bank[%d]\n",__FUNCTION__, dev_id, bank);
		return 0;
	}
...
void s5pv210_reserve_bootmem(void)
{
	...
	for (i = 0; i < nr_devs; i++) {
		...
		printk(KERN_INFO "s5pv210: %lu KB system memory reserved "
			"for %s at bank[%d] 0x%08x\n", (unsigned long) mdev->memsize/1024,
