...
static int hkdkc110_cam0_power(int onoff)
{      //-->GPD0.1  P2V8_EN ->1.8v and 2.8V on off
	int err;
	struct regulator *cam_regulator; //ldo6 2.8V

	/* Camera A */
	err = gpio_request(S5PV210_GPD0(1), "GPD0");
	if (err)
		printk(KERN_ERR "#### failed to request GPD0.1 for CAM_2V8 and 1.8V\n");

	s3c_gpio_setpull(S5PV210_GPD0(1), S3C_GPIO_PULL_NONE);
	cam_regulator = regulator_get(NULL, "vddcam");
	if (IS_ERR(cam_regulator)) {
      	printk(KERN_ERR "failed to get resource %s\n", "vddcam");
      	return PTR_ERR(cam_regulator);
	}
...
#endif
#ifdef S5K4EAGX_ENABLED
static struct s5k4eagx_platform_data s5k4eagx_plat = {
	.default_width = 1920,
	.default_height = 1080,
...
static struct s3c_platform_camera s5k4eagx = {
	...
	.initialized	= 0,
	.cam_power	= hkdkc110_cam0_power,//smdkv210_cam0_power,
};
#endif
...
{
	return	gpio_get_value(GPIO_CHARGER_DONE) ? 0 : 1;
}

static int max8677a_charger_online(void)
{
	return	gpio_get_value(GPIO_CHARGER_ONLINE) ? 0 : 1;
}
...
#endif	//defined(CONFIG_FB_S3C_LP101WH1)	
...
static struct i2c_board_info i2c_devs2[] __initdata = {
	...
	{ 	
		I2C_BOARD_INFO("wm8991", (0x34 >> 1)),
	}
};

//----------------------------------------------------------------------------------------
//
// YAMAHA Sensor Interface
//
//----------------------------------------------------------------------------------------
...
#define		GPIO_SENSORS_SDA	S5PV210_GPB(2)
#define		GPIO_SENSORS_SCL	S5PV210_GPB(0)

static struct 	i2c_gpio_platform_data 	i2c_sensors_platdata = {
	.sda_pin = GPIO_SENSORS_SDA, // gpio number
	.scl_pin = GPIO_SENSORS_SCL,
	.udelay  = 15,
	.sda_is_open_drain = 0,
	.scl_is_open_drain = 0,
...
static struct 	platform_device 	hkdkc110_device_i2c_sensors = {
	.name 	= "i2c-gpio",
	.id  	= 4, // adepter number
	.dev.platform_data = &i2c_sensors_platdata,
};
...
#endif	//#if defined(CONFIG_HKDKC110_BOARD) && defined(CONFIG_INPUT_YAMAHA_SENSORS) && !defined(CONFIG_INPUT_BMA150_SENSOR)
...
{
	pmem_pdata.start = (u32)s3c_get_media_memory_bank(S3C_MDEV_PMEM, 2);
	pmem_pdata.size = (u32)s3c_get_media_memsize_bank(S3C_MDEV_PMEM, 2);

	pmem_gpu1_pdata.start = (u32)s3c_get_media_memory_bank(S3C_MDEV_PMEM_GPU1, 2);
	pmem_gpu1_pdata.size = (u32)s3c_get_media_memsize_bank(S3C_MDEV_PMEM_GPU1, 2);

	pmem_adsp_pdata.start = (u32)s3c_get_media_memory_bank(S3C_MDEV_PMEM_ADSP, 2);
	pmem_adsp_pdata.size = (u32)s3c_get_media_memsize_bank(S3C_MDEV_PMEM_ADSP, 2);
}
#endif
...
EXPORT_SYMBOL(hkdkc110_5V_power);
...
static struct s3c_platform_fb lms350df_data __initdata = {
	.hw_ver			= 0x62,
	.nr_wins 		= 5,
	.default_win 	= CONFIG_FB_S3C_DEFAULT_WINDOW,
	.swap 			= FB_SWAP_WORD | FB_SWAP_HWORD,
	.reset_lcd		= lms350df_reset_lcd,
};
#endif
...
static struct platform_device hkdkc110_rfkill = {
    .name = "hkdkc110_rfkill",
    .id = -1,
};
#endif

#if defined(CONFIG_HKDKC110_BOARD)
static struct platform_device hkc1xx_sysfs = {
    .name = "hkc1xx-sysfs",
    .id = -1,
};
#endif
...
{
	pmem_pdata.start = s3c_get_media_memory_bank(S3C_MDEV_PMEM, 2);
	pmem_pdata.size = s3c_get_media_memsize_bank(S3C_MDEV_PMEM, 2);
}
#endif
...
MACHINE_START(SMDKC110, "HKDKC110")
