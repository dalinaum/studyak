...
extern unsigned char get_bt_wifi_status(void);
...
#if defined(CONFIG_FB_S3C_LMS350DF)
...
#elif defined(CONFIG_FB_S3C_LP101WH1)
...
int s3cfb_backlight_on(struct platform_device *pdev)
{
	...
	hkdkc110_3V3_power(1);

	gpio_direction_output(S5PV210_GPD0(2), 1); /* LED_EN (SPI1_MOSI) */
	gpio_direction_output(S5PV210_GPD0(1), 1); /* BL pwm High */
