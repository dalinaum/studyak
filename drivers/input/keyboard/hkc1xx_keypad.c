...
static void	hkc1xx_keypad_port_init(void)
{
	(*(unsigned long *)S5PC11X_GPH2CON)  = 0x00000000;
	(*(unsigned long *)S5PC11X_GPH2PUD)  = 0x00000540;
	
	(*(unsigned long *)S5PC11X_GPH0CON) &= (~0x000000F0);	
	(*(unsigned long *)S5PC11X_GPH0PUD) &= (~0x0000000C);
}
...
static int	hkc1xx_keypad_get_data(void)
{
	...
	key_data  	 = (~(*(unsigned long *)S5PC11X_GPH2DAT)) & 0x07;
	key_data 	|=   (*(unsigned long *)S5PC11X_GPH2DAT) & 0x28;
	
	temp 	  	 =   (*(unsigned long *)S5PC11X_GPH0DAT) << 3;
	temp		 =   (~temp) & 0x10;
	key_data 	|=     temp;
...
//	set_bit(EV_REP, hkc1xx_keypad.driver->evbit);	// Repeat Key
	...
	printk("--------------------------------------------------------\n");
	printk("HardKernel : HKDKC1XX Test driver initialized!! Ver 1.0\n");
	printk("--------------------------------------------------------\n");

#if defined(DO_NOT_SLEEP_FLAG)
	printk("--------------------------------------------------------\n");
	printk("\nDO NOT SLEEP FLAG Enable!!\n\n");
	printk("--------------------------------------------------------\n");
