...
static struct 	delayed_work dwork_vbus_detect;
static bool 	g_busbconnect;
...
//---------------------------------------------------------------------------------
//---------------------------------------------------------------------------------
int	hkdkc110_vbus_detect(struct delayed_work *dwork)
{
	struct s3c_udc *dev = the_controller;
	int err; 

	if (gpio_is_valid(S5PV210_GPB(4))) {
		err = gpio_request(S5PV210_GPB(4), "GPB4");
		if(err)	return err;

		if((gpio_get_value(S5PV210_GPB(4)))&&!g_busbconnect) {
            clk_enable(otg_clock);
            udc_enable(dev);
			g_busbconnect=true;
		}
		else if((!gpio_get_value(S5PV210_GPB(4)))&&g_busbconnect) {
            udc_disable(dev);
            clk_disable(otg_clock);
			g_busbconnect=false;
		}
	...
	schedule_delayed_work(&dwork_vbus_detect, msecs_to_jiffies(500));
...
//---------------------------------------------------------------------------------
//---------------------------------------------------------------------------------
static void s3c_udc_vbus_detect_init(void)
{
	gpio_request(S5PV210_GPB(4), "GPB4");
	s3c_gpio_setpull(S5PV210_GPB(4), S3C_GPIO_PULL_NONE);
	gpio_direction_input(S5PV210_GPB(4));
	if(gpio_get_value(S5PV210_GPB(4)))	g_busbconnect = true;
	else 								g_busbconnect = false;
	gpio_free(S5PV210_GPB(4));
	
	INIT_DELAYED_WORK(&dwork_vbus_detect, hkdkc110_vbus_detect);
	schedule_delayed_work(&dwork_vbus_detect, msecs_to_jiffies(500));
}
//---------------------------------------------------------------------------------
//---------------------------------------------------------------------------------
